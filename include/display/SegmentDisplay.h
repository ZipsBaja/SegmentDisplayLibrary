...
{
private:
    std::vector<TM1637_device_t*> devices;
    ...
    struct _BreakAnim
    {
        EventSource* source; // non-const ref
        int action_id;
        bool flag = false;
    };
...
public:
    SegmentDisplay(SegmentDisplaySettings& settings);
    SegmentDisplay(SegmentDisplaySettings* settings, size_t display_amount);
    ...
    void DisplayAnimationAll(const ArrayView2D<uint32_t>& frames, float target_fps, EventSource& event_device, bool reverse = false);
    void DisplayAnimationAll(const ArrayView<uint32_t>& frames, float target_fps, EventSource& event_device, bool reverse = false);
    void DisplayAnimationAll(const uint32_t** frames, size_t frame_count, size_t display_count, float target_fps, EventSource& event_device, bool reverse = false);
    void DisplayAnimationAll(const uint32_t* frames, size_t frame_count, float target_fps, EventSource& event_device, bool reverse = false);
    void DisplayAnimation(size_t index, const ArrayView<uint32_t>& frames, float target_fps, EventSource& event_device, bool reverse = false);
    void DisplayAnimation(size_t index, const uint32_t* frames, size_t frame_count, float target_fps, EventSource& event_device, bool reverse = false);
};